    ...
    public static Boolean expenseCheck(JSONObject expenseToCheck){
        ...
        for(int i = 0; i < expenses.length(); i++){
            ...
            if(expense.get("name").equals(expenseToCheck.get("name"))){
                String date = expenseToCheck.getString("date");
        ...
        LocalDate date = LocalDate.parse(expenseToCheck.getString("date"));
        Float amount = expenseToCheck.getFloat("amount");
        Expense newExpense = new Expense(name, category, date, amount);
    ...
    public static JSONArray getExpense(String name){
        ...
        for(int i = 0; i < expenses.length(); i++){
            ...
            if(expense.get("name").equals(name)){
                ...
                msg = "The amount of "+name+" is " + df.format(new Expense().amountOfExpense(help)*100.0) + " % of your current planned monthyl budget!";
                result.put(expense);
                break;
            }
        }
